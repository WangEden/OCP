...
#* ============================================
    if True:
        with open('./waverider/Planform_volume_section.txt','w+') as f:
            f.write('Planform  '+ str('%.4f' % planform) +'\n')
            f.write('Volume    '+ str('%.4f' % volume) +'\n')
            f.write('Section   '+ str('%.4f' % section) +'\n')
        
        # trailing
        with open('waverider/trailing_tec.dat', 'w+') as f2:
        ...
        # tecplot
        with open('./waverider/waverider_tec.dat','w+') as f4:
        ...
        with open('./waverider/lower_surface.xyz','w+') as f1:
            f1.write('1\n')
        ...
        with open('./waverider/upper_surface.xyz','w+') as f1:
            f1.write('1\n')
            f1.write(str(n_x) +'  '+ str(nu) + '  1\n')
            for i in [0,2,1]:
                for j in range(nu):
        ...
        with open('./waverider/TE.txt','w') as f1:
            for j in range(0,nu):
        ...
        with open('./waverider/LE.txt','w') as f1:
            for j in range(0,nu):
                ...
                f1.write('\n')
        with open('./waverider/SLOPE.txt','w') as f1:
            slope=(Lower_surface[0][-1][1]-Lower_surface[0][-2][1])/(Lower_surface[0][-1][0]-Lower_surface[0][-2][0])
            f1.write(str(slope))

        # igs
        plot3d_to_igs(fname='./waverider/upper_surface')
        plot3d_to_igs(fname='./waverider/lower_surface')